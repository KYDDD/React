...
function Counter({ children = "1" }: CounterProps) {
  ...
  // TODO 2. 증감치가 수정되면 1초후에 증감치 만큼 1회 자동 증가(handleUp() 호출)

  // useEffect(() => {
  //   setTimeout(() => {
  //     handleUp();
  //   }, 1000);
  //   console.log("dependencies에 값을 지정하면 컴포넌트가 업데이트 될 때 지정한 값중 하나라도 수정되었을 경우 호출됨");
  // }, [step]);
  ...
  useEffect(() => {
    ...
    return () => {
      //cleanup(컴포넌트가 언마운트될 떄 호출, setup 함수에서 생성한 자원을 해제하는 코드 작성
  ...
  //     handleUp();
  //   }, 1000);
  //   console.log("dependencies에 빈배열을 지정하지 않으면 마운트된 후와 업데이트된 후에 호출됨");
  // });
